...
type Connection struct {
	...
	connectAddr               string
	state                     int32
	//is a server side connection or client side connection. we should record it,cause client and server shared code.
	...
	iConn       InterfaceNet
	//lock to write data out.
	mutex sync.RWMutex

	//todo, inside build callback map.
...
func (c *Connection) WriteMessage(msg *PackHead) (n int, err error) {
	...
	if c == nil {
		log.Println("write to nil connection")
...
func (c *Connection) WriteMessageWithCallback(msg *PackHead, cb DefNetIOCallback) (n int, err error) {
	...
	if c == nil {
		...
		return 0, errors.New("try to write to nil connection" + reflect.TypeOf(c.iConn).String())
	}
	if c.state != ConnectionStateConnected {
		return 0, errors.New("try to write to a con which not est")
	}
	if c.netProtocol == NetProtocolTypeTCP {
		...
		if n, err = c.connWebSock.Write(data);err!=nil{
			log.Println("ws write data out failed", n, err)
		}
		return
	}
}
...
func (c *Connection) WriteMessageWaitResponseWithinTimeLimit(msg *PackHead, timeLimitMillisecond int64) (ackMsg *PackHead, isTimeOut bool) {
	...
	if c.state != ConnectionStateConnected {
		log.Println("connection not connected yet.")
		return nil, true
	}
	if c.netProtocol == NetProtocolTypeTCP {
...
func (c *Connection) PrintNetProtocol() {
	log.Println("is this a tcp protocol ?", c.netProtocol == NetProtocolTypeTCP)
}
